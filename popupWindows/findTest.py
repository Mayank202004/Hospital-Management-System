...
class findTests(tk.Tk,):
    ...
    def create_widgets(self):
        self.title("Find Tests")
        self.geometry("600x500")
        self.configure(bg="cornflowerblue")

        tk.Label(self, text="Find Tests", font=("Helvetica", 15, "bold")).place(x=220, y=10)
        ...
        self.searchbycbox=bs.Combobox(frametop,values=['Test Name','Department ID','Department Name'])
        self.searchbycbox.set('Test Name') # set default as Name 
        ...
        self.table = ttk.Treeview(tableframe, column=('TestID','TestName','DepartmentName'), xscrollcommand=s.set, yscrollcommand=s2.set)
        ...
        self.table.column('TestID', width=40)
        self.table.column('TestName', width=250)
        self.table.column('DepartmentName', width=250)
    ...
    def fetch_details(self):
        ...
        if(searchby=="Test Name"):
            searchby="TestName"
        elif(searchby=="Department ID"):
            searchby="DepartmentID"
        elif(searchby)=="Department Name":
            searchby="DepartmentName"
        value = self.searchby_text.get("1.0", "end-1c")
        data = self.db.search_tests(searchby,value)
        for tests in data:
            self.table.insert('', 'end', values=tests)
    ...
    def on_select(self, event=None):
        ...
        if selection:
            ...
            self.parent_window.setTest(id, name)
